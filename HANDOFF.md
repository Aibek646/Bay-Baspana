...
## Приложение для магазинов (Capacitor)

Код один, версии две. Собираются разными командами:

| Команда           | Что получается                                  |
| ----------------- | ----------------------------------------------- |
| `npm run build`   | сайт на Vercel: цена, Kaspi, «Я оплатил»        |
| `npm run android` | сборка для Google Play, копируется в `android/` |

Разница — переменная `VITE_STORE_BUILD=1` и флаг `STORE_BUILD` в `src/config.ts`.
В магазинной версии `SubscribePage` показывает «Доступ закрыт» без цены, без номера
и без WhatsApp: Apple и Google запрещают не сторонний платёж сам по себе, а любой
призыв платить мимо их встроенных покупок. Номер телефона на экране — такой призыв.
Доступ в этой версии открывает риелтор, вне приложения.

**Никогда не делать `cap sync` после обычного `npm run build`** — увезёт в магазин
экран с Kaspi. Для этого и существует команда `npm run android`, она делает оба шага.

Проверить, что в сборке нет платёжной части:

```
npm run android && grep -c "Kaspi" dist/assets/index-*.js   # должно быть 0
```

Сервис-воркер в магазинной сборке отключён (`disable` в `vite.config.ts`): приложение
и так стоит на телефоне, а его кеш умеет намертво подсунуть старую версию.

Иконки: источники в `assets/` (`icon.png`, `icon-foreground.png`, `icon-background.png`,
`splash.png`). Пересобрать все размеры — `npx @capacitor/assets generate --android`.

`appId` в `capacitor.config.ts` — `kz.baybaspana.app`. После публикации в магазине
не меняется никогда: для Google это имя приложения, другое имя = другое приложение.

**Перед отправкой на ревью** нужен тестовый аккаунт с открытым доступом, логин и пароль
отдаются в заявке. Ревьюер видит только экран входа, дальше не пройдёт — и откажет.

### Подпись и загрузка в Google Play

Файл для загрузки собирается одной командой:
...
Ключ подписи: `~/.android-keys/bay-baspana-upload.jks`, пароли — в
`~/.gradle/gradle.properties`. В репозитории их нет и быть не должно
(`*.jks` в `.gitignore`). **Потерять нельзя**: без этого ключа обновление
приложения в Play не загрузить. Копию файла и пароля держать отдельно от
ноутбука. Если ключа в настройках нет, `bundleRelease` соберёт неподписанный
файл, а не упадёт — так проект собирается у любого, без чужих ключей.

`versionCode` в `android/app/build.gradle` при каждой загрузке в Play надо
увеличивать на единицу: Play не принимает второй файл с тем же номером.

Сборка идёт на Java 21 (`org.gradle.java.home` в `~/.gradle/gradle.properties`).
Java 25, которую приносит Android Studio, Gradle 8.14 не понимает —
падает с `Unsupported class file major version 69`.
